# main.py — Pygbag-ready, local-assets-only
# Runs in browser via: pygbag --build .
# Scene: night city, searchlights, planes + bombs + AA, 1 US rooftop flag,
#        logo slides up after 10s then BAKED into background (no FPS hit).
...
def load_surface(path: str, has_alpha: bool, max_side: Optional[int] = None) -> pygame.Surface:
    """Load an image from local assets and optionally scale to max_side."""
    try:
        img = pygame.image.load(path)
        img = img.convert_alpha() if has_alpha else img.convert()
    except Exception:
        # Simple fallback surface if asset missing
        img = pygame.Surface((120, 40), pygame.SRCALPHA if has_alpha else 0)
        col = (200,200,200,200) if has_alpha else (200,200,200)
        img.fill(col)
        img = img.convert_alpha() if has_alpha else img.convert()
...
def prerender_background(sw: int, sh: int) -> Tuple[pygame.Surface, int, List[Tuple[int,int]], List[Building]]:
    """Draws sky, stars, moon, skyline once; returns bg surface, city base Y, damage spots, buildings list."""
...
async def game():
    ...
    # Windowed base; pygbag scales canvas to page
    ...
    for path, has_alpha in PLANE_FILES:
        plane_sprites.append(load_surface(path, has_alpha, max_side=PLANE_MAX_SIDE))

    try:
        flag_img = load_surface(US_FLAG_FILE, True, max_side=120)
    except Exception:
        flag_img = None
    ...
    while running:
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                elif e.key == pygame.K_f: show_fps = not show_fps
        ...
        for f in fires:
            ...
            pygame.draw.circle(screen, (255,240,170,200), max(1,int(size*0.7)))
